...
export async function addToCart(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const cartItem = await updateOrCreateCartItem(tx, cart.id, productId, storeId)
...
export async function decrementCart(prisma: PrismaClient, data: updateCartPayload): Promise<CartItem> {
...
export async function incrementCart(prisma: PrismaClient, data: updateCartPayload): Promise<CartItem> {
  const cart = await prisma.cartItem.update({
    where: { cart: { userId: data.userId }, id: data.cartItemId },
    data: {
      quantity: {
...
export async function deleteCartItem(prisma: PrismaClient, data: updateCartPayload): Promise<void> {
  await prisma.cartItem.delete({
    where: { cart: { userId: data.userId }, id: data.cartItemId }
  })
}

export async function deleteCart(prisma: PrismaClient, data: deleteCartPayload): Promise<void> {
